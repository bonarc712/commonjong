...
public class MahjongShantenCounter
{
	...
	public static int shantenCount(int[] hand, int groupCount){
		// INIT
		int retVal = 8; int tCount = 0; int pCount = 0;
		...
		if (hand.length > 3) {
		    ...
		    for (int k=j+1; k<hand.length && (k-j<5); k++) {
		        ...
		        if (isSet(hand[i],hand[j],hand[k])) i++; j=999; k=999;
		    }}}
		}
		// PROTOGROUP COUNT
		if (hand.length > 1) {
		      ...
		      for (int i = 0; i<hand.length-1; i++) {
		          if(isPair(hand[i], hand[i+1])) {pCount++;i++;}
		          else if(i<hand.length-2 && isPair(hand[i], hand[i+2])) {pCount++;i=i+2;}
		          else if(isTaatsu(hand[i], hand[i+1])) {tCount++;i++;}
		      }
		  }
		  // 5 to 7 pairs, immediate return on 7
		  if (pCount==7) return -1; if (pCount>4&&hand.length>=13) retVal = Math.min(retVal, 6-pCount); 
		  // Standard count: 8 - 2*g - max(4-g,p+t) - min(1,max(0,p+t-(4-g)))  
		  if(pCount+tCount>Math.floor(hand.length/3)) {
		      if (pCount>0) retVal = (int) Math.min(retVal, 8 - 2*groupCount - Math.floor(hand.length/3) - 1);  
		      else retVal = (int) Math.min(retVal, 8 - 2*groupCount - Math.floor(hand.length/3) - 0);
		  } else retVal = Math.min(retVal, Math.min(8 - 2*groupCount - pCount - tCount, 6 - pCount));
	...
	private static int countDiffTerminals(int[] hand) {
		...
		if (isTerminal(hand[0])) res++;
		// Check non-pairs ones/nines, winds/dragons
		for (int i=1; i<hand.length; i++) {
			if (isTerminal(hand[i]) && hand[i-1] != hand[i]) res++;
			if (isTerminal(hand[i]) && hand[i-1] != hand[i]) res++;
		}
		return res;
	}
	...
	private static int terminalPairs(int[] hand) {
		...
		for (int i=1; i<hand.length; i++) {
			...
			// Distinct pair logic.
			if (isTerminal(hand[i]) && hand[i-2] == hand[i]) res--;
		}
		return res;
	}
	...
	private static boolean isRun(int a, int b, int c) {
		...
		if (a+1 == b && b+1 == c && sameSuit(a, c)) return true;
		return false;
	}
	...
	private static boolean isTaatsu(int a, int b) {
		...
		if ((a+1 == b || a+2 == b) && sameSuit(a, b) && b<27) return true;
		return false;
	}
	...
	private static int[] insertTile(int[] hand, int a) {
		...
		for (int i=0; i<hand.length; i++) {
			...
			if (hand[i] >= a) newHand[i+1] = hand[i];
		}
		newHand[temp] = a;
		return newHand;
	}
	...
	private static boolean flushHand(int[] hand) {
		...
		for (int i = 0; i<hand.length; i++) {
			if (hand[i] < 9) man++;
			else if (hand[i] < 18) pin++;
			else if (hand[i] < 27) so++;
		}
		// Reduce result to one or leave at zero.
		man = Math.min(1, man);
		pin = Math.min(1, pin);
		so = Math.min(1, so);
		if (man+pin+so > 1) return false;
		return true;
	}

	private static boolean pureFlushHand(int[] hand) {
		if (sameSuit(hand[0],hand[hand.length-1])) return true;
		return false;
	}

	private static boolean isTanyao(int[] hand) {
		for (int i = 0; i<hand.length; i++) {
			if (isHonor(hand[i])) return false;
			else if (isEnd(hand[i])) return false;
		}
		return true;
	}
